...
MSC_PORT = 4254
TARGET_MSISDN = "1001" 
DURASI_TAHAN = 14.5  # Tahan sinyal tepat 14.5 Detik

def get_time():
    # Menampilkan waktu presisi (Jam:Menit:Detik.Milidetik)
    return datetime.now().strftime("%H:%M:%S.%f")[:-3]

def run_precision_loop(target_msisdn):
    ...
    try:
        ...
        # Tunggu sebentar untuk start awal
        tn.read_until(b"# ", timeout=1)

        while True:
            # 1. TAHAN SINYAL (HOLD PHASE)
            # Sinyal dibiarkan memancar stabil
            print(f"[{get_time()}] ➤ [ON] Memancar {DURASI_TAHAN}s...")
            time.sleep(DURASI_TAHAN)

            # 2. FASE REFRESH (ZERO DELAY)
            ...
            # Kita cuma butuh memastikan buffer kosong
            tn.read_until(b"# ", timeout=0.1)

            # C. SPAM START (RECONNECT PHASE)
            # Terus kirim perintah START tanpa jeda tidur sampai MSC menerima
            while True:
                tn.write(f"subscriber msisdn {target_msisdn} silent-call start any speech-fr\n".encode('ascii'))
                ...
                # Jika output "busy", loop akan otomatis ulang seketika (Brutal Mode)
